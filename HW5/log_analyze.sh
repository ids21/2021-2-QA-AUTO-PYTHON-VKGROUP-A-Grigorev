...
echo "Count of  requsts:" > "$RESULT"
wc -l "$ACCESS_LOG" | awk '{print $1}' >> "$RESULT"

echo "Count of requsts by HTTP method:" >> "$RESULT"
cat "$ACCESS_LOG" | awk '{print $6}' | sed 's/^"//' | sort | uniq -c | \
	sort -rnk1 | awk '{printf "%d - %s\n", $1, $2}' >> "$RESULT"
...
echo "TOP -5 largest requests that failed wilth (4XX) error:" >> "$RESULT"
